...
public class MultiItemTool_QT extends MultiItemTool {
    ...
    public boolean onEntitySwing(EntityLivingBase entity, ItemStack stack) {
        ...
        if (behaver != null) for (IBehavior<MultiItem> tBehavior : behaver)
            try {
    ...
    public boolean onBlockStartBreak(ItemStack itemstack, int X, int Y, int Z, EntityPlayer player) {
        IToolStats stats = this.getToolStatsInternal(itemstack);
        IAOE_Tool tool = null;
        try {
            tool = (IAOE_Tool) stats;
        } catch (Exception e) {
            // was not an instance
        }
        if (tool == null) return false;
        return tool.onBlockStartBreak(itemstack, X, Y, Z, player);
    }
